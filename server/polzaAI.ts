...
ПРАВИЛА ОЦЕНКИ:
1. Если ответ содержит случайные буквы, бессмысленный набор символов — это НЕПРАВИЛЬНЫЙ ответ.
2. Если ответ слишком короткий (менее 50 символов) и не раскрывает суть — это НЕПРАВИЛЬНЫЙ ответ.
3. Если ответ показывает понимание задачи, предлагает конкретные решения, учитывает контекст — это ПРАВИЛЬНЫЙ ответ.

ВАЖНО:
- Если ответ ПРАВИЛЬНЫЙ: отметь конкретные удачные моменты в решении, похвали за понимание проблемы или оригинальный подход.
- Если ответ НЕПРАВИЛЬНЫЙ: НЕ давай прямой ответ на задачу! Укажи направление, в котором стоит подумать. Задай наводящий вопрос или предложи посмотреть на проблему с другой стороны.
...
Отвечай только JSON, без дополнительного текста.`;
  ...
  try {
    ...
    console.error("Error evaluating task solution with Polza AI:", error);
  }

  return {
    isCorrect: false,
    feedback: "Не удалось проверить ответ. Попробуйте позже.",
  };
}
